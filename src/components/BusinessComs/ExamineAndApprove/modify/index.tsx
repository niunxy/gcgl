...
import { Button, Form, Select, Input, Checkbox, message, Row, Col, Card } from 'antd'
...
class Modify extends React.Component<IProps, IState> {
            ...
            if (!err) {
                InfoAction.Approve(values).then((res: any) => {
                    if (res.status === 200) {
                        ...
                        // this.props.history.replace({ pathname: '/index/examineAndApprove/approva' })
                        this.handleReset()
                        // this.props.init()
                    } else {
    ...
    render() {
        ...
        return (
            <div className='modify-box'>
                <Card title='变更申请' className='card'>
                    <Form labelCol={{ span: 6 }} wrapperCol={{ span: 12 }} onSubmit={this.handleSubmit}>
                        ...
                        <Form.Item label='是否加急处理'>
                            {getFieldDecorator('urgent', { initialValue: this.state.urgent })(
                                <Checkbox checked={this.state.urgent} onChange={this.urgentHandle}>特急提醒</Checkbox>
                            )}
                        </Form.Item>
...
const WrappedApp = Form.create<IProps>({ name: 'coordinated' })(Modify)
