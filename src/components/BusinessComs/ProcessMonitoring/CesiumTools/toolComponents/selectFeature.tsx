...
export default class SelectFeature extends React.Component<IProps> {
    ...
    componentWillReceiveProps() {
        if (this.props.selectedFeature) {
            if (this.props.selectedFeature.selectedEntityFlag !== this.selectedEntityFlag) {
                this.isLoading = true
                this.selectedEntityFlag = this.props.selectedFeature.selectedEntityFlag
            }
            this.setDataPanel(this.props.selectedFeature.properties)
        } else {
            data = []
    ...
    // 关闭组件
    closeVisible = () => {
        this.props.setDataPanelShow(false)
    }
    ...
    setWindowPosition = (panelLeft, panelTop) => {
        const cesiumContainerDom = document.getElementById(this.props.parentDomId) as HTMLElement
        ...
        const attributePanelHeight = 200
        let panelLocation = {
            panelLeft,
            panelTop
        }
    ...
    render() {
        ...
        this.setLoading()
        return (
            <div className= '_AttributePanel' style={{left: windowPosition.panelLeft, top: windowPosition.panelTop}}>
                <div className='_AttributePanelTitle'><span>属性信息</span><Icon type='close' onClick={this.closeVisible} style={{position: 'absolute', right: '10px', top: '10px', zIndex: 10, opacity: 0.5}} /></div>
                <div className='_AttributePanelBody'><Table columns={columns} dataSource={data} showHeader={false} pagination={false} scroll={{y: 140}} loading = {this.isLoading}/></div>
            </div>
        )
    }
}
