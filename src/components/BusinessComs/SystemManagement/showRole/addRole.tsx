...
class AddRole extends React.Component<IProps, IState> {
            ...
            if (res.status === 200) {
                ...
                res.data.menus.forEach((item) => {
                    if (item.pid) {
                        ...
                        const arr = res.data.menus.filter((value) => {
                        ...
                        arr.forEach((list) => {
                            const arr1 = res.data.menus.filter((list1) => {
                                return list.id === list1.pid
                            })
                            if (arr1.length === 0) {
                                if (!menus.includes(list.id)) {
                                    menus.push(list.id)
                                }
                            }
                            arr1.forEach((list2) => {
                                if (!menus.includes(list2.id)) {
                                    menus.push(list2.id)
                                }
                            })
                        })
    ...
    render() {
        ...
        return !loginUtils.isLogin()
            ? <Redirect to='/login' />
            : (
                <div style={{ margin: '0 -24px' }}><Form onSubmit={this.handleSubmit} labelCol={{ span: 6 }} wrapperCol={{ span: 16 }}>
